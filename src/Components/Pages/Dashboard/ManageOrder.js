...
const ManageOrder = () => {
    ...
    const {user} = useAuth()
    useEffect(() => {
        fetch(`https://shrouded-hollows-39187.herokuapp.com/orders`)
        .then(res => res.json())
        .then(data => {
            setOrders(data)
            console.log(data)
    ...
    const updateStatus = (id, index )=> {
        ...
        fetch(`https://shrouded-hollows-39187.herokuapp.com/updatestatus/${id}`, {
            method: 'PUT', 
    ...
    const handleDelete = id => {
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
                fetch(`https://shrouded-hollows-39187.herokuapp.com/deleteorder/${id}`, {
                    method: 'DELETE', 
                    headers: {'content-type' : 'application/json'}
                })
                .then(res => res.json())
